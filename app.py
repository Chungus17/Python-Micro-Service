    ...
    if response.status_code == 200:
        try:
            ...
            if task_function == "hourly orders":
                ...
                return jsonify({"message": "Wrong function called 😂"})
...
def send_email(excel_file, subject, clientName):
    try:
        ...
        to_email = "finance@tryverdi.com"
        cc_emails = ["admin@tryverdi.com", "mrharoonkhan11@gmail.com"]
...
def data_analysis():
    ...
    thread = threading.Thread(
        target=getData,
        args=(start_date, end_date, filter_by, clientName, task_function),
    )
    thread.start()
...
def get_Average_Fare(data, clientName):
    ...
    for user, amounts in user_amounts.items():
        ...
        final_data.append({"User Name": user, "Average Fare": round(avg, 2)})
...
def get_Total_Fare(data, clientName):
    ...
    for user, amounts in user_amounts.items():
        ...
        final_data.append({"User Name": user, "Total Fare": round(total, 2)})
...
def get_Amount_Ranges(data, clientName):
    ...
    range_labels = [
        f"{lo} - {hi if hi != float('inf') else '+'}" for lo, hi in range_limits
    ]
    ...
    send_email(
        output_file_name, subject="Total orders in fare ranges", clientName=clientName
    )
...
def get_Pickup_Counts_Per_Area(data, clientName):
    ...
    send_email(
        output_filename, subject="Pickups per area in every hour", clientName=clientName
    )
